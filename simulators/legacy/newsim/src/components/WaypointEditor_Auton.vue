  ...
  <div class="wrap">
    <div class="box">
      ...
      <table>
        <tr>
          <td>
            <button v-on:click="parseWaypoint('w')">Add Waypoint</button>
          </td>
          <td>
        ...
        <tr>
          <td>
            <button v-on:click="parseWaypoint('o')">Add Obstacles</button>
          </td>
          <td>
            <button v-on:click="parseWaypoint('a')">Add AR Tag</button>
          </td>
        </tr>
        </table>

      <Checkbox v-bind:name="'Add waypoint by mouseclick?'" v-on:toggle="toggleWaypointCheckbox()" />
      <Checkbox v-bind:name="'Add obstacle by mouseclick?'" v-on:toggle="toggleWaypointCheckbox()" />
      <Checkbox v-bind:name="'Add AR Tag by mouseclick?'" v-on:toggle="toggleWaypointCheckbox()" />
    ...
    <div class="box">
      ...
      <draggable v-model="storedWaypoints" class="dragArea" draggable=".item'">
        <WaypointItem v-for="waypoint, i in storedWaypoints" :key="i" v-bind:waypoint="waypoint" v-bind:list="0" v-bind:index="i" v-on:delete="deleteItem($event)" v-on:toggleSearch="toggleSearch($event)" v-on:toggleGate="toggleGate($event)" v-on:add="addItem($event)" v-on:convertDMS="convertWaypointDMS($event, 'w')"/>
      </draggable>
    </div>
    <div class="box">
      <h3>Current Course</h3>
      <draggable v-model="route" class="dragArea" draggable=".item'">
        <WaypointItem v-for="waypoint, j in route" :key="j" v-bind:waypoint="waypoint" v-bind:list="1" v-bind:index="j" v-on:delete="deleteItem($event)" v-on:toggleSearch="toggleSearch($event)" v-on:toggleGate="toggleGate($event)" v-on:add="addItem($event)" v-on:convertDMS="convertWaypointDMS($event, 'w')"/>
      </draggable>
    </div>
    <div class="box">
      <h3>Current Obstacles</h3>
        <draggable v-model="obstacles" class="dragArea" draggable=".item'">
        <WaypointItem v-for="waypoint, k in obstacles" :key="k" v-bind:waypoint="waypoint" v-bind:list="2" v-bind:index="k" v-on:delete="deleteItem($event)" v-on:toggleSearch="toggleSearch($event)" v-on:toggleGate="toggleGate($event)" v-on:add="addItem($event)" v-on:convertDMS="convertWaypointDMS($event, 'o')"/>
      </draggable>
    </div>
    <div class="box">
      <h3>Current AR Tags</h3>
      <draggable v-model="ARtags" class="dragArea" draggable=".item'">
        <WaypointItem v-for="waypoint, m in ARtags" :key="m" v-bind:waypoint="waypoint" v-bind:list="3" v-bind:index="m" v-on:delete="deleteItem($event)" v-on:toggleSearch="toggleSearch($event)" v-on:toggleGate="toggleGate($event)" v-on:add="addItem($event)" v-on:convertDMS="convertWaypointDMS($event, 'a')"/>
      </draggable>
    </div>
  </div>
</template>
...
import ObstacleItem from './ObstacleItem_Auton.vue'
import ARTagItem from './ARTagItem_Auton.vue'
...
export default {
  ...
  methods: {
    ...
    deleteItem: function (payload) {
      if(payload.list === 0) {
        ...
        this.obstacles.splice(payload.index, 1)
      } else if(payload.list === 3){
        this.ARtags.splice(payload.index, 1)
      }
    },

    // Add item from all waypoints div to current waypoints div
    addItem: function (payload, inputType) {
       if(payload.list === 0) {
        this.route.push(this.storedWaypoints[payload.index])
      } else if(payload.list === 1) {
        this.storedWaypoints.push(this.route[payload.index])
      } else if(inputType === 'o') {
        this.obstacles.push(this.storedWaypoints[payload.index])
      } else if(inputType === 'a') {
        this.ARtags.push(this.storedWaypoints[payload.index])
      }
    ...
    addWaypoint: function (lat, lon, inputType) {
    ...
    dropWaypoint: function () {
      ...
      this.addWaypoint(lat, lon, 'w')
    },

    convertWaypointDMS: function(payload, inputType) {
      ...
      this.addWaypoint(parseCoordinate(payload.lat), -parseCoordinate(payload.lon), inputType)  
    ...
    parseWaypoint: function (inputType) {
      const parseCoordinate = function (input) {
        const nums = input.split(" ")
        switch (nums.length) {
          case 1:
      ...
      this.addWaypoint(parseCoordinate(this.lat), -parseCoordinate(this.lon), inputType) 
    },
  ...
  watch: {
    ...
    obstacles: function (newList) {
      this.setWaypointList(newList)
    },

    ARtags: function (newList) {
      this.setWaypointList(newList)
    },
  },
