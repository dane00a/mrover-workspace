<template>
  <div class="wrap">
    <div class="box">
      ...
      <button v-on:click="parseWaypoint()">Add Obstacle</button>
      <Checkbox v-bind:name="'Add obstacle by mouseclick?'" v-on:toggle="toggleWaypointCheckbox()" />
    </div>
    <div class="box">
        <h3>This is currently empty, awaiting a purpose in life :)</h3>
    </div>
    <div class="box">
      <h3>All Obstacles</h3>
      <draggable v-model="storedWaypoints" class="dragArea" draggable=".item'">
        <ObstacleItem v-for="waypoint, i in storedWaypoints" :key="i" v-bind:waypoint="waypoint" v-bind:list="0" v-bind:index="i" v-on:delete="deleteItem($event)" v-on:toggleSearch="toggleSearch($event)" v-on:toggleGate="toggleGate($event)" v-on:add="addItem($event)" v-on:convertDMS="convertWaypointDMS($event)"/>
      </draggable>
    </div>
    <div class="box">
      <h3>Current Obstacles</h3>
      <draggable v-model="route" class="dragArea" draggable=".item'">
        <ObstacleItem v-for="waypoint, i in route" :key="i" v-bind:waypoint="waypoint" v-bind:list="1" v-bind:index="i" v-on:delete="deleteItem($event)" v-on:toggleSearch="toggleSearch($event)" v-on:toggleGate="toggleGate($event)" v-on:add="addItem($event)" v-on:convertDMS="convertWaypointDMS($event)"/>
      </draggable>
    </div>
  </div>
</template>
...
export default {
  ...
  created: function () {
    ...
    interval = window.setInterval(() => {
        ...
        let course = {
            num_obstacle: this.route.length,
            obstacles: _.map(this.route, (waypoint) => {
        ...
        obstacle.hash = fnvPlus.fast1a52(JSON.stringify(course));
        obstacle.type = 'Obstacle'
        this.$parent.publish('/obstacles', obstacles)
