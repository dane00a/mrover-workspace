<template>
  <div class="wrap">
    ...
    <l-map v-on:click="setMouseCoords" ref="map" class="map" :zoom="15" :center="center">
      ...
      <l-marker :lat-lng="waypoint.latLng" :icon="waypointIcon" v-for="(waypoint,index) in obstacleList" :key="waypoint.id">
        <l-tooltip :options="{ permanent: 'true', direction: 'center'}"> {{ index }} </l-tooltip>
      </l-marker>

      <l-marker :lat-lng="waypoint.latLng" :icon="waypointIcon" v-for="(waypoint,index) in ARTags" :key="waypoint.id">
        <l-tooltip :options="{ permanent: 'true', direction: 'center'}"> {{ index }} </l-tooltip>
      </l-marker>
